...
if [ -e "/opt/local/bin/port" ]; then
	sudo port install automake cmake doxygen libelf libmpc libusb wget xz
	
	install_libusb
elif [ -e "/usr/local/bin/brew" ]; then
	CURRENT_USER=$(stat -f '%Su' /dev/console)
	sudo -u $CURRENT_USER brew install automake cmake doxygen libelf libmpc libusb libusb-compat wget xz
else
